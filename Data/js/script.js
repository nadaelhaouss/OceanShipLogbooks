...
// Change wind icon's direction based on wind info 
...
var start= L.geoJSON(start, {
    pointToLayer: function(feature, latlng) {return L.marker(latlng, { icon: ShipIcon });},
    onEachFeature: onEachFeature
});

var routesGeoJSON2 = L.geoJSON(routes, {style: { opacity: '0.1', color: '#016c59'},
    onEachFeature: createRoutesPopup
});

var routesGeoJSON = L.geoJSON(routes, {style: { color: '#016c59'},
    onEachFeature: createRoutesPopup
});
...
// Add filter option By ship name, departure year or trip ID
...
// Add event listener when clicked on departure point
...
var map = L.map('map', {
    ...
    zoom: 3,
    layers: [darkLayer, clusterLayer]
});




//Add spatial query (find shortest and longest trip)
